<template>
    <div class="relative font-serif">
        <button class="relative flex items-center text-left border border-gray-200 text-base font-serif rounded-lg px-4 py-1 uppercase hover:bg-white hover:border-gray-500" @click="isOpen = !isOpen" :class="{ 'border-gray-500': isOpen, 'bg-white': isOpen, 'bg-gray-200': !isOpen, 'z-75': isOpen }" style="width: 150px">
            <span class="inline-block mr-1 flex-1">Language</span>
        ...
        <div class="w-1/3 absolute left-0 rounded-lg bg-white z-100 overflow-hidden mt-2 border border-gray-500" v-if="isOpen">
            <button v-for="(language, code) in languages" class="w-full flex items-center space-x-2 text-base text-left px-4 py-1 uppercase" :class="code === selected ? 'bg-blue-400 text-white' : 'hover:bg-blue-400 hover:text-white'" @click="select(code)">
