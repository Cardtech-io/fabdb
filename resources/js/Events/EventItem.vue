<template>
    <li class="block flex odd:bg-gray-100 hover:bg-gray-200 items-center">
        <span class="block flex-initial p-4">
            <router-link :to="{ name: 'events.edit', params: { event: event.slug } }">
        ...
        <div class="flex-auto py-4">
            <router-link :to="{ name: 'events.edit', params: { event: event.slug } }">
                ...
                <p class="text-base">{{ time.calendar(event.when) }}</p>
                <p v-if="event.manager.slug == user.slug" class="text-sm">You are managing this event.</p>
            </router-link>
        </div>

        <a href="" class="text-center py-6 px-8 flex-initial border-l border-gray-200 link" @click.prevent="copyShareURL(event)">
        ...
        <span class="text-center py-6 px-8 flex-initial border-l border-gray-200">
