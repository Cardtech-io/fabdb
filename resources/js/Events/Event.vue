            ...
            <div class="container sm:mx-auto px-4 flex items-center">
                <ul class="flex-auto">
                    <li class="inline-block text-center font-serif uppercase">
                        <a href="" class="block p-4" @click.prevent="setType('constructed')" :class="activeType('constructed')">
                    ...
                    <li class="inline-block text-center font-serif uppercase">
                        <a href="" class="block p-4" @click.prevent="setType('draft')" :class="activeType('draft')" :disabled="typeAvailable('draft')">
                            <svg class="fill-current h-10 w-10 mx-auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    ...
                    <li class="inline-block text-center font-serif uppercase">
                        <a href="" class="block p-4" @click.prevent="setType('sealed')" :class="activeType('sealed')" :disabled="typeAvailable('sealed')">
                            <svg class="fill-current h-10 w-10 mx-auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
            ...
            <div class="container sm:mx-auto p-4 py-8 md:flex md:space-x-8">
                <div class="md:w-1/2 bg-gray-100 dark:bg-gray-800 p-4">
                    <form @submit.prevent="saveEvent">
                        <div class="w-full mb-2">
                            ...
                            <input v-model="event.name" class="input focus:border-gray-500 py-2 px-4 rounded-lg mb-2" required>
                        </div>

                        <div class="w-full mb-2">
                            <label class="block font-serif uppercase tracking-wide mb-1">Event fee</label>
                            <input type="number" v-model="event.fee" step=".01" min="0" class="input focus:border-gray-500 py-2 px-4 rounded-lg mb-2">
                        </div>

                        <div class="w-full mb-2">
                            <label class="block font-serif uppercase tracking-wide mb-1">When</label>
                            <datetime v-model="event.startsAt" input-class="input focus:border-gray-500 py-2 px-4 rounded-lg mb-2" format="yyyy-MM-dd HH:mm:ss" type="datetime" :use12-hour="true" :minute-step="15"></datetime>
                        </div>

                        <div class="w-full mb-2">
                            <label class="block font-serif uppercase tracking-wide mb-1">Description</label>
                            <markdown-editor :value="event.description" ref="markdownEditor"/>
                        </div>
                ...
                <div class="md:w-1/2 mt-8 md:mt-0">
                    ...
                    <div class="text-right mt-8" v-if="event.slug">
                        <cancel-event :event="event"/>
                    </div>

                    <div class="mt-8">
                        <manage-players :event="event"/>
                    </div>
                </div>
            </div>
        </div>
            ...
            activeType(type) {
                if (this.typeAvailable(type)) {
                    return {
                        ...
                        'text-gray-800 dark:text-white': this.event.type == type,
                        'text-red-400': this.event.type != type,
                        'hover:text-gray-800 dark:hover:text-gray-300': this.event.type != type,
                    }
                } else {
