<template>
    <div>
        ...
        <div class="block lg:hidden relative font-serif">
            <button class="relative flex items-center text-left text-base font-serif rounded-lg px-4 py-1 uppercase hover:bg-white dark:hover:bg-gray-600" @click="isOpen = !isOpen" :class="{ 'bg-white dark:bg-gray-600': isOpen, 'bg-white': isOpen, 'bg-gray-200 dark:bg-black': !isOpen, 'z-75': isOpen }" style="width: 150px">
            ...
            <div class="w-full absolute right-0 rounded-lg bg-white dark:bg-gray-800 z-100 overflow-hidden mt-2 shadow-lg" v-if="isOpen">
                <button v-for="(mode, option) in modes" class="flex items-center space-x-2 w-full text-base text-left px-4 py-1 uppercase" :class="option === mode ? 'bg-blue-400 text-white' : 'hover:bg-blue-400 hover:text-white'" @click="select(option)">
                    <icon :size="5" v-html="mode.icon"/>
                    <span>{{ mode.name }}</span>
                </button>
            </div>
