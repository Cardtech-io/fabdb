<template>
    <div class="relative font-serif mr-2 sm:mr-4">
        <button class="relative text-left border border-gray-200 text-base font-serif rounded-lg px-2 sm:px-4 py-1 sm:py-2 uppercase hover:bg-white hover:border-gray-500 flex" @click="isOpen = !isOpen" :class="{ 'border-gray-500': isOpen, 'bg-white': isOpen, 'bg-gray-200': !isOpen, 'z-75': isOpen }" style="width: 150px">
        ...
        <div class="w-full absolute right-0 rounded-lg bg-white z-100 overflow-hidden mt-2 border border-gray-500" v-if="isOpen">
            <button v-for="(text, mode) in modes" class="w-full text-base text-left px-2 sm:px-4 py-1 sm:py-2 hover:bg-blue-400 hover:text-white uppercase" @click="select({ mode })">{{ text }}</button>
        </div>
    </div>
</template>
